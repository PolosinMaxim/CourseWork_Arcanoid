...
brickColors = [WHITE, lightBLUE, RED, GREEN]
...
width_const = WIDTH // 2 - brick_width
...
class Game():
    ...
    def ChangeScreenSize(self):
        global WIDTH, HEIGHT, width_const, brick_width, brick_height, brick_field_start, min_bita_width, max_bita_width
        WIDTH, HEIGHT = Screen_Sizes[Size_ID]
        brick_width =  WIDTH // brick_n - brick_border
        brick_height =  HEIGHT // 20 - brick_border
        width_const = WIDTH // 2 - brick_width
    ...
    def create_bricks(self):
        self.bricks = []
        l = jfile['levels'][levels[level_id]]
##        rows = 0
##        columns = 0
        #l = l['structure']
        for i in range(len(l)):
            i_obj = l[i]
            #brick_n = l['bricks_n']
            if len(i_obj) >= brick_n: i_obj = i_obj[:brick_n]
            else: i_obj = i_obj * (brick_n // len(i_obj)) + i_obj[:brick_n % len(i_obj)]
            for j in range(len(i_obj)):
                j_obj = i_obj[j]
...
##                    rows = 0
##                    columns += 1
                #print(l.index(i), i.index(j))
                if not j_obj: continue
                brick = Brick(j_obj)
                brick.x = j * (brick_width + brick_border)
                brick.y = brick_field_start + i * (brick_height + brick_border)
...
##                self.bricks.append(brick)
    ...
    def create_labels(self):
        ...
        self.level_label = TextObject((WIDTH - brick_width) // 2,
                                      5,
                                      lambda: f'УРОВЕНЬ {levels[level_id]}',
                                      GREEN,
                                      'Arial',
                                      brick_height)
        ...
        self.gameover_label = TextObject(width_const,
                                      brick_height * 2,
    ...
    def records_show(self):
        player_list = [i.split(chr(9)) for i in open("BestRecords.txt", "r")]
        self.firstbest_label = TextObject(width_const,
                                      HEIGHT * 3 // 5,
        ...
        if len(player_list) >= 2:
            self.secondbest_label = TextObject(width_const,
                                      HEIGHT * 13 // 20,
        ...
        if len(player_list) >= 3:
            self.thirdbest_label = TextObject(width_const,
                                      HEIGHT * 7 // 10,
...
class Brick():
    def __init__(self, initType: int):
        self.tipe = initType - 1
##        try: c = int(initType)
##        except ValueError:
...
##        else: self.tipe = c
    ...
    def update(self):
        pass
    def draw(self,screen):
        pygame.draw.rect(screen, brickColors[self.tipe % len(brickColors)], (self.x, self.y, brick_width, brick_height))
